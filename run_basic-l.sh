...
do
    ...
    do
        ...
        do
            export K=$k
            export P=$p
            python -m scripts.train --config training_config/basic-l.jsonnet --serialization-dir model_logs/"$metric"-basic-ladder-movies-K"$K"P"$P"-namefree  --environment LEO  --device 0 --seed 6 --override
        done
    done
done
