    ...
    do
        ...
        do
            ...
            do
                export K=$k
                export P=$p
                python -m scripts.train --config training_config/basic-l-div.jsonnet --serialization-dir model_logs/"$metric"-basic-ladder-movies-K"$K"P"$P"-namefree-div"$doc_kl"  --environment LEO_DIV  --device 0 --seed 6 --override
            done
        done
    done
done
