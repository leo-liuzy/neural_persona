...
if __name__ == "__main__":
    ...
    for docid, data in corexmls_from_files(xml_docs):
        ...
        for entity in entities:
            names = [m['name'] for m in entity['mentions']]
            best_name = None
            ...
            for name in names:
                if not any("DATE" == part[2] for part in name) and \
                    not any("PRP" in part[1] for part in name) and \
                        len(name) >= 2 and \
                        any(part[0] in docs[docid] for part in name):
                    tmp_name = " ".join(part[0] for part in name)
                    if best_name is None or len(tmp_name) < len(best_name):
                        best_name = tmp_name

            if best_name is None:
                continue
            else:
                new_entity = dict(mentions=list(set(m['sentence'] for m in entity['mentions'])),
                                  name=best_name)
                slim_entities.append(new_entity)

        cur_covered_mentions = len(set(itertools.chain(*[e['mentions'] for e in slim_entities])))
        num_covered_mentions += cur_covered_mentions
        num_uncovered_mentions += len(sentences) - cur_covered_mentions

        if len(slim_entities) == 0:
            num_ignored_doc += 1
            continue
    ...
    print(f"""
    Total Doc: {len(xml_docs)} 
    Processed Doc: {len(processed_corpus)}
    Missed Doc: {num_missed_doc}
    Ignored Doc: {num_ignored_doc}
    
    Among included docs...
    Total Entity: {num_missed_entity + num_entity}
