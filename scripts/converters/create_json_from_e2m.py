...
def write_json_from_e2m_file_ladder(files: List[str], data_set: str = "train"):
    doc_cover_pairs = []
    if not os.path.exists(f"{new_data_home}/ladder/"):
        os.makedirs(f"{new_data_home}/ladder/")
    # assert model_type in ["avitm", "partial-gen"]
    assert data_set in ["train", "dev", "test"]
    data = []  # {"docs": [], "entities": [], "doc_reindex_table": {i: i for i in range(len(files))}}
    mentions = []
    for filename in tqdm(files):
        ...
        entities = content["entities"]
        new_entities = [{"entity_label": entity["MID"], "entity_text_ids": list(set(entity["mentions"]))} for entity in entities]
        all_mentions_idx = list(itertools.chain(*[entity["mentions"] for entity in entities]))
        doc_cover_pairs.append((doc_len, len(set(all_mentions_idx))))
        mentions.append(" ".join([doc_sentences[i] for i in all_mentions_idx]))
    ...
    with open(f"{new_data_home}/{data_set}_mentions.jsonl", "w") as f:
        json.dump(mentions, f)
    with open(f"{new_data_home}/{data_set}_doc_cover_pairs.json", "w") as f:
        json.dump(doc_cover_pairs, f)
...
write_json_from_e2m_file_ladder(train_files, "train")
write_json_from_e2m_file_ladder(dev_files, "dev")
write_json_from_e2m_file_ladder(test_files, "test")
