...
def main(args):
    ...
    parser.add_option('--entity', action="store_true",
                      help='if use entity mentions as mini-doc')
...
def preprocess_data(train_infile,
    ...
    if stopwords == 'mallet':
        ...
        stopword_list = read_text(os.path.join('neural_persona', 'common', 'stopwords', 'mallet_stopwords.txt'))
    elif stopwords == 'snowball':
        print("Using snowball stopwords")
        stopword_list = read_text(os.path.join('neural_persona', 'common', 'stopwords', 'snowball_stopwords.txt'))
    elif stopwords is not None:
        print("Using custom stopwords")
        stopword_list = read_text(os.path.join('neural_persona', 'common', 'stopwords', stopwords + '_stopwords.txt'))
    else:
        stopword_list = []
    ...
    if entity:
        ...
        for item in all_items:
            ...
            for entity in entities:
                mentions = " ".join([" ".join(tokenized_text[idx]) for idx in entity["entity_text_ids"]])
                all_texts.append(mentions)
