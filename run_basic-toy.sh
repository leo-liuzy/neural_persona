...
export DATA_DIR="$(pwd)/examples/toy/basic"
# export VOCAB_SIZE=1995
export VOCAB_SIZE=3000
export LAZY=1   
export DOC_LINEAR_SCALING=2000
...
do
    ...
    do
        ...
        do
            export K=$k
            export P=$p
            python -m scripts.train --config training_config/basic-toy.jsonnet --serialization-dir model_logs/"$metric"-basic-ladder-toy-K"$K"P"$P"-div"$DOC_LINEAR_SCALING" --environment LEO  --device 0 --seed 6 --override
        done
    done
done
