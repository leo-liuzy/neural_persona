    ...
    do
        ...
        do
            ...
            do
                export K=$k
                export P=$p
                python -m scripts.train --config training_config/bamman.jsonnet --serialization-dir model_logs/"$metric"-bamman-movies-K"$K"P"$P"-namefree-div"$doc_kl"  --environment BAMMAN --device 0 --seed 6 --override
            done
        done
    done
done
