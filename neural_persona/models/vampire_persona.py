...
class VAMPIRE(Model):
    ...
    def __init__(self,
        ...
        self._cur_epoch = 0
        self._cur_topic_npmi = 0.0
        self._cur_persona_npmi = 0.0
        self.batch_num = 0
    ...
    def update_topics_and_persona(self, epoch_num: Optional[List[int]]) -> None:
        """
        ...
        if epoch_num and epoch_num[0] != self._metric_epoch_tracker:
            ...
            if self.track_topics:
                ...
                with open(persona_filepath, 'w+') as file_:
                    file_.write(perspna_table)

                # save topic2persona model
                topic2persona_dir = os.path.join(os.path.dirname(self.vocab.serialization_dir), "topic2persona")
                if not os.path.exists(topic2persona_dir):
                    os.mkdir(topic2persona_dir)
                ser_dir = os.path.dirname(self.vocab.serialization_dir)
    ...
    def update_npmi(self) -> None:
        ...
        if self.track_npmi and self._ref_vocab and not self.training and not self._npmi_updated:
            ...
            self._cur_persona_npmi = self.compute_npmi(personas[1:])
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        self.update_topics_and_persona(epoch_num)
        self.update_npmi()
        ...
        self.metrics['d_npmi'] = self._cur_topic_npmi
        self.metrics['e_npmi'] = self._cur_persona_npmi
