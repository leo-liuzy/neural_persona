...
class BasicL(Model):
    ...
    def __init__(self,
        ...
        if doc_kl_weight_annealing == "linear":
            ...
            self._doc_kld_weight = float(1 / (1 + np.exp(-self._doc_sigmoid_weight_1 * (1 - self._doc_sigmoid_weight_2))))
        ...
        if entity_kl_weight_annealing == "linear":
            ...
            self._entity_kld_weight = float(1 / (1 + np.exp(-self._entity_sigmoid_weight_1 * (1 - self._entity_sigmoid_weight_2))))
        ...
        if not epoch_num:
            ...
            if _epoch_num != self._kl_epoch_tracker:
                ...
                if self._doc_kl_weight_annealing == "linear":
                    ...
                    self._doc_kld_weight = float(1 / (1 + np.exp(- self._doc_sigmoid_weight_1 * (self._cur_epoch - self._doc_sigmoid_weight_2))))
                ...
                if self._entity_kl_weight_annealing == "linear":
                    ...
                    self._entity_kld_weight = float(1 / (1 + np.exp(- self._entity_sigmoid_weight_1 * (self._cur_epoch - self._entity_sigmoid_weight_2))))
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        if not self.training:
            self._doc_kld_weight = 1.0  # pylint: disable=W0201
        else:
